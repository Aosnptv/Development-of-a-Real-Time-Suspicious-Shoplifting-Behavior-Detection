...
CAMERA_SOURCES = {
    "Camera 1 (Main Shelf)": 0,
    "Camera 2 (Blind Spot)": 1
}
...
class CameraStream:
    def __init__(self, source):
        ...
        if not self.cap.isOpened():
            ...
            cv2.putText(self.frame, f"CAMERA SOURCE [{source}] OFFLINE", (60, 240), 
...
def process_frame_pipeline(frame, camera_name, current_frame_number):
    ...
    if obj_results.boxes is not None:
        for box in obj_results.boxes:
            ...
            if cls == 60 and conf > 0.35: 
                ...
                cv2.putText(frame, f"DETECTION ZONE ({int(conf*100)}%)", (detected_shelf_zone[0], detected_shelf_zone[1] - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.45, (255, 0, 255), 2)
                break 
            ...
            if int(box.cls[0]) == 0:  
                ...
                if len(keypoints) > 10:
                    if keypoints[9][0] > 0 and keypoints[9][1] > 0:
                        ...
                        cv2.circle(frame, (int(left_hand[0]), int(left_hand[1])), 5, (0, 255, 255), -1)
                    if keypoints[10][0] > 0 and keypoints[10][1] > 0:
                        right_hand = (keypoints[10][0], keypoints[10][1])
                        cv2.circle(frame, (int(right_hand[0]), int(right_hand[1])), 5, (0, 255, 255), -1)
                ...
                label = f"SUSPECT ID: {person_id} | Risk: {current_score}/5 ({current_state})"
    ...
    cv2.putText(frame, f"Frame: {current_frame_number}", (530, 465), cv2.FONT_HERSHEY_SIMPLEX, 0.45, (0, 255, 255), 2)
    return frame